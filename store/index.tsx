...
type Reducer<P> = (state: State, dispatchSideEffect: SideEffect, payload: P) => void
...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [state, dispatch] = useReducer((state: State, action: Actions | SideEffectsActions): State => {
    ...
    reducersMap[action.type](
      newState,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (action: SideEffectsActions) => dispatch({ type: `side-effect-${action.type}`, payload: (action as unknown as any).payload } as unknown as Actions),
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
